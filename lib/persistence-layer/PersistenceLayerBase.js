...
class PersistenceLayerBase {
     ...
     * 5. Its processing was not finished
     * 6. Its name is not in `excludeTasksWithNames`
     * 7. Is the task with the earliest due date of all tasks that fulfill the previous points
     *
     ...
     *
     * @param excludeTasksWithNames - Names of the tasks that shall be excluded
     * @returns task instance or Promise
     */
    getNextPendingTask(excludeTasksWithNames) {
        throw new Error('Missing implementation for PersistenceLayerBase.getNextPendingTask(...)')
    }
    ...
    setTaskProcessingTime({ task, until }) {
        throw new Error('Missing implementation for PersistenceLayerBase.setTaskProcessingTime(...)')
    }

    /**
     * Cancels the execution of a task which makes it pending again.
     *
     * If its execution failed the task record shall be retained for an indefinite time
     * to give the admin the chance to analyze the failure.
     *
    ...
    cancelTaskProcessing({ task, executionFailed, err }) {
        throw new Error('Missing implementation for PersistenceLayerBase.cancelTaskProcessing(...)')
    }
    ...
    finishTaskProcessing({ task, retain, retainUntil, storeOutput, output }) {
        throw new Error('Missing implementation for PersistenceLayerBase.finishTaskProcessing(...)')
    }
     ...
     * 1. All tasks that are due or past due and are neither being processed nor their execution failed,
     * 2. All tasks that are being processed and will become pending after the processing timeout,
     * 3. All tasks that will be due in the future, and
     * 4. All tasks with their names in `excludeTasksWithNames` must be ignored.
     *
     ...
     */
    getNextPollingTime(excludeTasksWithNames) {
        throw new Error('Missing implementation for PersistenceLayerBase.getNextPollingTime(...)')
    }
     ...
     */
    storeNewTask({ task }) {
        throw new Error('Missing implementation for PersistenceLayerBase.storeNewTask(...)')
    }
