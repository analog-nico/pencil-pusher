...
class TaskDefinition {
    ...
    constructor(name, options) {
        ...
        if (_.has(this._definition.execution, 'maxConcurrentTasks') && (!_.isInteger(this._definition.execution.maxConcurrentTasks) || this._definition.execution.maxConcurrentTasks <= 0)) {
            throw new TypeError('options.execution.maxConcurrentTasks must be a positive integer.')
        }
    ...
    getMaxConcurrentTasks() {
        return this._definition.execution.maxConcurrentTasks || null
    }
    ...
    getProcessingUntil() {
        ...
        if (_.isFinite(this._definition.execution.completesWithin)) {
            return [nowUnix, nowUnix + this._definition.execution.completesWithin]
        } else {
            return [nowUnix, now.add(this._definition.execution.completesWithin).unix()]
        }
